...
@section('content')
    ...
    <h3>Combined Monthly Summary Chart</h3>
    <canvas id="combinedMonthlyChart" width="400" height="200"></canvas>
</div>

<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
<script>
    document.addEventListener("DOMContentLoaded", function () {
        const ctx = document.getElementById('combinedMonthlyChart').getContext('2d');

        // Prepare data for the chart
        const monthlyOrders = @php echo json_encode($monthlyorder); @endphp;
        const monthlyPurchase = @php echo json_encode($monthly_purchase); @endphp;

        // Extract labels from the data (assuming both datasets have the same months)
        const labels = monthlyOrders.map(order => order.month_year);

        // Data for Orders
        const ordersData = monthlyOrders.map(order => order.order_count); // Quantity of orders
        const ordersTotalAmount = monthlyOrders.map(order => parseFloat(order.total_amount)); // Total Amount for Orders

        // Data for Purchases
        const purchaseCounts = monthlyPurchase.map(purchase => purchase.purchase_count); // Quantity of purchases
        const purchasesTotalAmount = monthlyPurchase.map(purchase => parseFloat(purchase.total_amount)); // Total Amount for Purchases

        const combinedMonthlyChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: labels,
                datasets: [
                    {
                        label: 'Order Count',
                        data: ordersData,
                        backgroundColor: 'rgba(75, 192, 192, 0.5)', // Color for Order Count line
                    ...
                    },
                    {
                        ...
                        backgroundColor: 'rgba(255, 159, 64, 0.5)', // Color for Order Total Amount line
                    ...
                    },
                    {
                        ...
                        data: purchaseCounts,
                        backgroundColor: 'rgba(54, 162, 235, 0.5)', // Color for Purchase Count line
                    ...
                    },
                    {
                        ...
                        data: purchasesTotalAmount,
                        backgroundColor: 'rgba(255, 99, 132, 0.5)', // Color for Purchase Total Amount line
